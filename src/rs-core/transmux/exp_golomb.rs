...
impl<'a> ExpGolomb<'a> {
    ...
    pub fn len(&self) -> usize {
        return 8 * self.working_bytes_available;
    }

    pub fn bits_available(&self) -> u64 {
        return 8 * (self.working_bytes_available as u64) + self.working_bits_available;
    }
    ...
    pub fn skip_leading_zeros(&mut self) -> u64 {
        ...
        self.load_word();
        return working_bits + self.skip_leading_zeros();
    }
    ...
    pub fn read_signed(&mut self) -> i32 {
        ...
        if 0x01 & valu != 0 {
            ...
            -1 * (valu >> 1) // divide by two then make it negative
        }
    }
