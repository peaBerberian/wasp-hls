...
QUnit.test("triggers log for non-id3/invalid data", function (assert) {
  ...
  const zeroFrames = new Uint8Array(
    stringToInts("ID3").concat([
      0x03,
...
QUnit.test("parses simple ID3 metadata out of PES packets", function (assert) {
  var events = [],
    wxxxPayload = [
      ...
      stringToInts("http://example.com/ad?v=1234&q=7")
    ), // value
    id3Bytes,
    size;
  ...
  id3Bytes = new Uint8Array(
    stringToInts("ID3").concat(
      ...
      id3Frame("WXXX", wxxxPayload), // value
      // frame 1
      // custom tag
      id3Frame("XINF", [
        0x04,
...
QUnit.test("parses TXXX frames without null terminators", function (assert) {
  ...
  assert.equal(
    events[0].frames[0].description,
    "get done",
    "parsed the description"
  );
  assert.deepEqual(
    JSON.parse(events[0].frames[0].data),
    { key: "value" },
    "parsed the data"
  );
});
...
QUnit.test("parses WXXX frames", function (assert) {
  var events = [],
    url = "http://example.com/path/file?abc=7&d=4#ty";
  metadataStream.on("data", function (event) {
    events.push(event);
  });
...
QUnit.test(
  "parses TXXX frames with characters that have a single-digit hexadecimal representation",
  function (assert) {
    var events = [],
...
QUnit.test("parses PRIV frames", function (assert) {
  var events = [],
    payload = stringToInts(
      "arbitrary data may be included in the payload " + "of a PRIV frame"
    );
  ...
  metadataStream.push({
    ...
    data: new Uint8Array(
      id3Tag(
        id3Frame("PRIV", stringToCString("priv-owner@example.com"), payload)
      )
    ),
  });
...
QUnit.test("parses tags split across pushes", function (assert) {
  var events = [],
    ...
    payload = stringToInts(
      "A TS packet is 188 bytes in length so that it can" +
  ...
  assert.equal(events[0].frames.length, 1, "parsed a frame");
  assert.equal(
    events[0].frames[0].data.byteLength,
    payload.length,
    "collected data across pushes"
  );

  // parses subsequent fragmented tags
  tag = new Uint8Array(id3Tag(id3Frame("PRIV", owner, payload, payload)));
...
QUnit.test(
  "id3 frame is malformed first time but gets corrected in the next frame",
  function (assert) {
    var events = [],
      owner = stringToCString("owner@example.com"),
      payload = stringToInts(
...
QUnit.test("ignores tags when the header is fragmented", function (assert) {
  ...
  metadataStream.push({
    ...
    data: new Uint8Array(
      id3Tag(
        id3Frame("PRIV", stringToCString("owner2"), stringToInts("payload2"))
      )
    ),
  });
  assert.equal(events.length, 1, "parsed one tag");
...
QUnit.test("constructs the dispatch type", function (assert) {
  ...
  assert.equal(
    metadataStream.dispatchType,
    "1503020100",
    "built the dispatch type"
  );
});

QUnit.test(
  "should skip tag frame parsing on malformed frame, preserving previous frames",
  function (assert) {
    var events = [],
      validFrame = id3Frame(
        "TIT2",
...
QUnit.test("can parse APIC frame in web worker", function (assert) {
  ...
  worker.addEventListener("message", function (e) {
    ...
    assert.equal(
      e.data.frames[0].mimeType,
      "image/jpeg",
      'parsed MIME type is "image/jpeg"'
    );
    assert.equal(
      e.data.frames[0].pictureType,
      0x03,
      "parsed picture type is 0x03"
    );
    assert.equal(
      e.data.frames[0].description,
      "sample description",
      "parsed description"
    );
    assert.deepEqual(
      e.data.frames[0].pictureData,
      new Uint8Array(stringToInts("picture binary data")),
      "parsed picture data"
    );
    assert.equal(e.data.frames[1].key, "APIC", "frame key is APIC");
    assert.equal(e.data.frames[1].mimeType, "-->", 'parsed MIME type is "-->"');
    assert.equal(
      e.data.frames[1].pictureType,
      0x04,
      "parsed picture type is 0x04"
    );
    assert.equal(
      e.data.frames[1].description,
      "sample description 2",
      "parsed description"
    );
    assert.equal(
      e.data.frames[1].url,
      "http://example.org/cover-back.jpg",
      "parsed picture data"
    );
    worker.terminate();
    done();
...
QUnit.test("can parse PRIV frames in web worker", function (assert) {
  ...
  worker.addEventListener("message", function (e) {
    ...
    assert.deepEqual(
      new Uint8Array(e.data.frames[0].data),
      new Uint8Array(payload),
      "parsed the frame private data"
    );
    worker.terminate();
...
QUnit.test("can parse TXXX frames in web worker", function (assert) {
  ...
  worker.addEventListener("message", function (e) {
    ...
    assert.deepEqual(
      JSON.parse(e.data.frames[0].data),
      { key: "value" },
      "parsed the data"
    );
    worker.terminate();
...
QUnit.test("should parse text frames in web worker", function (assert) {
  ...
  worker.addEventListener("message", function (e) {
    ...
    assert.equal(
      e.data.frames[0].values.length,
      1,
      "parsed value is an array of size 1"
    );
    assert.equal(
      e.data.frames[0].values[0],
      "sample song title",
      "parsed a non multiple strings value"
    );
    assert.equal(e.data.frames[1].key, "TIT3", "frame key is TIT3");
    assert.equal(
      e.data.frames[1].value,
      "sample title 1\0sample title 2",
      "parsed value"
    );
    assert.equal(
      e.data.frames[1].values.length,
      2,
      "parsed value is an array of size 2"
    );
    assert.equal(
      e.data.frames[1].values[0],
      "sample title 1",
      "parsed 1st multiple strings value"
    );
    assert.equal(
      e.data.frames[1].values[1],
      "sample title 2",
      "parsed 2nd multiple strings value"
    );
    worker.terminate();
    done();
  });
    ...
    data: new Uint8Array(
      id3Tag(
        ...
        id3Frame(
          ...
          stringToCString("sample title 1"),
          stringToCString("sample title 2")
        )
      )
    ),
  });
...
QUnit.test(
  "triggers special event after parsing a timestamp ID3 tag",
  function (assert) {
    ...
    assert.equal(
      frame.timeStamp,
      900000,
      "Initial timestamp fired and calculated correctly"
    );
