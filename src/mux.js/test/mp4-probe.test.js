...
QUnit.test("reads the timescale from an mdhd", function (assert) {
  ...
  // in the mdhd
  assert.deepEqual(
    probe.timescale(new Uint8Array(mp4Helpers.sampleMoov)),
    {
      1: 90e3,
      2: 90e3,
    },
...
QUnit.test("returns null if the tkhd is missing", function (assert) {
  assert.equal(
    probe.timescale(new Uint8Array(moovWithoutTkhd)),
    null,
    "indicated missing info"
  );
});

QUnit.test("returns null if the mdhd is missing", function (assert) {
  assert.equal(
    probe.timescale(new Uint8Array(moovWithoutMdhd)),
    null,
    "indicated missing info"
  );
});
...
QUnit.test(
  "startTime reads the base decode time from a tfdt",
  function (assert) {
    assert.equal(
      probe.startTime(
        {
...
QUnit.test(
  "startTime returns the earliest base decode time",
  function (assert) {
    assert.equal(
      probe.startTime(
        {
...
QUnit.test(
  "compositionStartTime calculates composition time using composition time" +
    "offset from first trun sample",
  function (assert) {
    assert.equal(
      probe.compositionStartTime(
        {
          1: 6,
          4: 3,
        },
...
QUnit.test(
  "compositionStartTime looks at only the first traf",
  function (assert) {
    assert.equal(
      probe.compositionStartTime(
        {
...
QUnit.test(
  "compositionStartTime uses default composition time offset of 0" +
    "if no trun samples present",
  function (assert) {
    assert.equal(
      probe.compositionStartTime(
        {
...
QUnit.test(
  "getTimescaleFromMediaHeader gets timescale for version 0 mdhd",
  function (assert) {
...
QUnit.test("can get ID3 data from a v0 EMSG box", function (assert) {
  ...
  assert.equal(
    emsgBoxes[0].cueTime,
    20,
    "got correct emsg cueTime value from v0 emsg"
  );
  assert.equal(
    emsgBoxes[0].duration,
    0,
    "got correct emsg duration value from v0 emsg"
  );
  assert.equal(emsgBoxes[0].frames[0].id, "PRIV", "got correct ID3 id");
  assert.equal(
    emsgBoxes[0].frames[0].owner,
    "priv-owner@example.com",
    "got correct ID3 owner"
  );
  assert.deepEqual(
    emsgBoxes[0].frames[0].data,
    new Uint8Array(id3.stringToInts("foo.bar.id3.com")),
    "got correct ID3 data"
  );
});
...
QUnit.test("can get ID3 data from a v1 EMSG box", function (assert) {
  ...
  assert.equal(
    emsgBoxes[0].cueTime,
    100,
    "got correct emsg cueTime value from v1 emsg"
  );
  assert.equal(
    emsgBoxes[0].duration,
    0.01,
    "got correct emsg duration value from v1 emsg"
  );
  assert.equal(emsgBoxes[0].frames[0].id, "TXXX", "got correct ID3 id");
  assert.equal(
    emsgBoxes[0].frames[0].description,
    "foo bar",
    "got correct ID3 description"
  );
  assert.deepEqual(
    JSON.parse(emsgBoxes[0].frames[0].data),
    { key: "value" },
    "got correct ID3 data"
  );
});
...
QUnit.test("can get ID3 data from multiple EMSG boxes", function (assert) {
  ...
  assert.equal(
    emsgBoxes[1].duration,
    0,
    "got correct emsg duration value from v0 emsg"
  );
  assert.equal(emsgBoxes[1].frames[0].id, "TXXX", "got correct ID3 id");
  assert.equal(
    emsgBoxes[1].frames[0].description,
    "foo bar",
    "got correct ID3 description"
  );
  assert.deepEqual(
    JSON.parse(emsgBoxes[1].frames[0].data),
    { key: "value" },
    "got correct ID3 data"
  );
});
...
moovWithoutMdhd = box(
  ...
  box(
    ...
    box(
      ...
      0x2c,
      0x00,
      0x00, // 300 in 16.16 fixed-point
      0x00,
      0x96,
      0x00,
      0x00
    ), // 150 in 16.16 fixed-point
    box(
      "mdia",
      box(
        "hdlr",
...
moofWithTfdt = box(
  ...
  box(
    ...
    box(
      ...
      0x00,
      0x00,
      0x01, // base_data_offset
      0x00,
      0x00,
      0x00,
      0x02, // sample_description_index
      0x00,
      0x00,
      0x00,
      0x03, // default_sample_duration,
      0x00,
      0x00,
      0x00,
      0x04, // default_sample_size
      0x00,
      0x00,
      0x00,
      0x05
    ...
    box(
      ...
      0x00,
      0x0f,
      0x01, // flags: dataOffsetPresent, sampleDurationPresent,
      ...
      0x00,
      0x00,
      0x00, // data_offset, no first_sample_flags
      // sample 1
      0x00,
      0x00,
      0x00,
      0x0a, // sample_duration = 10
      0x00,
      0x00,
      0x00,
      0x0a, // sample_size = 10
      ...
      0x00,
      0x00,
      0x0a, // signed sample_composition_time_offset = 10
      // sample 2
      0x00,
      0x00,
      0x00,
...
); // signed sample_composition_time_offset = 20
...
); // data_offset, no first_sample_flags
...
multiTraf = box(
  ...
  box(
    ...
    box(
      ...
      0x00,
      0x00,
      0x0b, // signed sample_composition_time_offset = 11
      // sample 2
      0x00,
      0x00,
      0x00,
...
); // signed sample_composition_time_offset = 5

multiMoof = moofWithTfdt.concat(
  box(
    "moof",
    box(
      "mfhd",
    ...
    box(
      ...
      box(
        ...
        0x00,
        0x00,
        0x14, // signed sample_composition_time_offset = 20
        // sample 2
        0x00,
        0x00,
        0x00,
...
); // signed sample_composition_time_offset = 10
v1boxes = box(
  "moof",
  box(
    "mfhd",
