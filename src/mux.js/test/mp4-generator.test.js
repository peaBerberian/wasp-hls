...
validateMvhd = function (mvhd) {
  ...
  QUnit.assert.equal(
    mvhd.duration,
    0xffffffff,
    "wrote the maximum movie header duration"
  );
  QUnit.assert.equal(
    mvhd.nextTrackId,
    0xffffffff,
    "wrote the max next track id"
  );
};
...
validateTkhd = function (tkhd, expected) {
  ...
  QUnit.assert.deepEqual(
    tkhd.flags,
    new Uint8Array([0, 0, 7]),
    "flags should QUnit.equal 7"
  );
  QUnit.assert.equal(
    tkhd.duration,
  ...
  QUnit.assert.equal(
    tkhd.width,
    expected.width || 0,
    "wrote width into the track header"
  );
  QUnit.assert.equal(
    tkhd.height,
    expected.height || 0,
    "wrote height into the track header"
  );
...
validateMdia = function (mdia, expected) {
  ...
  QUnit.assert.equal(
    mdia.boxes.length,
    3,
    "generated three track media sub boxes"
  );
...
QUnit.test("generates an initialization segment", function (assert) {
  var data = mp4.generator.initSegment([
      {
        id: 1,
        width: 600,
        height: 300,
  ...
  assert.equal(
    init[1].boxes[0].duration,
    0xffffffff,
    "wrote a maximum duration"
  );
  ...
  assert.equal(
    trak1.boxes[1].boxes[1].handlerType,
    "vide",
    "wrote the first track type"
  );
  ...
  assert.equal(
    trak2.boxes[1].boxes[1].handlerType,
    "soun",
    "wrote the second track type"
  );
...
QUnit.test("generates a minimal moof", function (assert) {
  ...
  assert.equal(
    moof[0].boxes[0].sequenceNumber,
    7,
    "included the sequence_number"
  );
  assert.equal(moof[0].boxes[1].type, "traf", "generated a traf box");
  assert.equal(
    moof[0].boxes[1].boxes.length,
    4,
    "generated track fragment info"
  );
  assert.equal(moof[0].boxes[1].boxes[0].type, "tfhd", "generated a tfhd box");
  assert.equal(
    moof[0].boxes[1].boxes[0].trackId,
    17,
    "wrote the first track id"
  );
  assert.equal(
    moof[0].boxes[1].boxes[0].baseDataOffset,
    undefined,
    "did not set a base data offset"
  );

  assert.equal(moof[0].boxes[1].boxes[1].type, "tfdt", "generated a tfdt box");
  assert.ok(
    moof[0].boxes[1].boxes[1].baseMediaDecodeTime >= 0,
  ...
  assert.equal(
    trun.samples[0].compositionTimeOffset,
    500,
    "wrote the composition time offset"
  );
  ...
  assert.equal(
    trun.samples[1].compositionTimeOffset,
    1000,
    "wrote the composition time offset"
  );
...
QUnit.test("can generate a traf without samples", function (assert) {
  ...
  assert.equal(
    moof[0].boxes[1].boxes[2].samples.length,
    0,
    "generated no samples"
  );
});

QUnit.test("generates an mdat", function (assert) {
  var data = mp4.generator.mdat(new Uint8Array([1, 2, 3, 4])),
