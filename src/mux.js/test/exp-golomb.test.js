...
QUnit.test("small numbers are coded correctly", function (assert) {
  ...
  while (i--) {
    ...
    assert.equal(
      expected[i][1],
      result,
      expected[i][0] + " is decoded to " + expected[i][1]
    );
  }
});
...
QUnit.test("skipBits correctly across word-boundaries", function (assert) {
  var expGolomb = new ExpGolomb(
    new Uint8Array([0x15, 0x00, 0x00, 0x28, 0x00, 0x0a, 0x00, 0x00])
  );
  assert.equal(
    expGolomb.readUnsignedExpGolomb(),
    9,
  ...
  assert.equal(
    expGolomb.readUnsignedExpGolomb(),
    4,
    "the second number is read"
  );
  expGolomb.skipBits(13); // Crosses word boundary
  assert.equal(
    expGolomb.readUnsignedExpGolomb(),
    4,
    "the third number is read"
  );
});

QUnit.test("parses a sequence parameter set", function (assert) {
  var sps = new Uint8Array([
      0x27, 0x42, 0xe0, 0x0b, 0xa9, 0x18, 0x60, 0x9d, 0x80, 0x35, 0x06, 0x01,
      0x06, 0xb6, 0xc2, 0xb5, 0xef, 0x7c, 0x04,
    ]),
    expGolomb = new ExpGolomb(sps);

  assert.strictEqual(
    expGolomb.readBits(8),
    0x27,
    "the NAL type specifies an SPS"
  );
  assert.strictEqual(expGolomb.readBits(8), 66, "profile_idc is 66");
  assert.strictEqual(
    expGolomb.readBits(4),
    0x0e,
    "constraints 0-3 are correct"
  );
  ...
  assert.strictEqual(
    expGolomb.readUnsignedExpGolomb(),
    0,
    "seq_parameter_set_id is 0"
  );
  assert.strictEqual(
    expGolomb.readUnsignedExpGolomb(),
    1,
    "log2_max_frame_num_minus4 is 1"
  );
  assert.strictEqual(
    expGolomb.readUnsignedExpGolomb(),
    0,
  ...
  assert.strictEqual(
    expGolomb.readUnsignedExpGolomb(),
    3,
    "log2_max_pic_order_cnt_lsb_minus4 is 3"
  );
  assert.strictEqual(
    expGolomb.readUnsignedExpGolomb(),
    2,
    "max_num_ref_frames is 2"
  );
  assert.strictEqual(
    expGolomb.readBits(1),
    0,
    "gaps_in_frame_num_value_allowed_flag is false"
  );
  assert.strictEqual(
    expGolomb.readUnsignedExpGolomb(),
    11,
    "pic_width_in_mbs_minus1 is 11"
  );
  assert.strictEqual(
    expGolomb.readUnsignedExpGolomb(),
    8,
    "pic_height_in_map_units_minus1 is 8"
  );
  assert.strictEqual(expGolomb.readBits(1), 1, "frame_mbs_only_flag is true");
  assert.strictEqual(
    expGolomb.readBits(1),
    1,
    "direct_8x8_inference_flag is true"
  );
