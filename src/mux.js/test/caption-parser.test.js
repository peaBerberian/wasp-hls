...
QUnit.test("parseTrackId for version 0 and version 1 boxes", function (assert) {
  ...
  assert.equal(
    v0Captions.captions[0].text,
    "test string #1",
    "got the expected version0 caption text"
  );
  assert.equal(
    v0Captions.captions[0].stream,
    "CC1",
    "returned the correct caption stream CC1"
  );
  assert.equal(
    v0Captions.captions[0].startTime,
    10 / 90000,
    "the start time for version0 caption is correct"
  );
  assert.equal(
    v0Captions.captions[0].endTime,
    10 / 90000,
    "the end time for version0 caption is correct"
  );
  assert.equal(v0Captions.captionStreams.CC1, true, "stream is CC1");
  assert.ok(!v0Captions.captionStreams.CC4, "stream is not CC4");
  ...
  assert.equal(
    v1Captions.captions[0].text,
    "test string #2",
    "got the expected version1 caption text"
  );
  assert.equal(
    v1Captions.captions[0].stream,
    "CC4",
    "returned the correct caption stream CC4"
  );
  assert.equal(
    v1Captions.captions[0].startTime,
    30 / 90000,
    "the start time for version1 caption is correct"
  );
  assert.equal(
    v1Captions.captions[0].endTime,
    30 / 90000,
    "the end time for version1 caption is correct"
  );
  assert.equal(v1Captions.captionStreams.CC4, true, "stream is CC4");
  assert.ok(!v1Captions.captionStreams.CC1, "stream is not CC1");
});
    ...
    [
      {
        level: "warn",
        message:
          "We've encountered a nal unit without data at 189975 for trackId 1. See mux.js#223.",
      },
    ],
...
version0Moof = box(
  ...
  box(
    ...
    box(
      ...
      0x00,
      0x00,
      0x00, // base_data_offset
      0x00,
      0x00,
      0x00,
      0x00, // sample_description_index
      0x00,
      0x00,
      0x00,
...
version1Moof = box(
  ...
  box(
    ...
    box(
      ...
      0x00,
      0x00,
      0x0a, // default_sample_duration = 10
      0x00,
      0x00,
      0x00,
      0x0a
    ...
    box(
      ...
      0x00,
      0x0c,
      0x05, // flags: dataOffsetPresent, sampleFlagsPresent,
      ...
      0x00,
      0x00,
      0x00, // data_offset, has first_sample_flags
      // sample 1
      0x00,
      0x00,
      0x00,
