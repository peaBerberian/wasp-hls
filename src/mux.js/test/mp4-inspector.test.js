...
var mp4 = require("../lib/mp4"),
  ...
  tkhd0 = box(
    ...
    0x2c,
    0x80,
    0x00, // 300.5 in 16.16 fixed point
    0x00,
    0x96,
    0x80,
    0x00
  ), // 150.5 in 16.16 fixed point
  mdhd0 = box(
    "mdhd",
    0x00, // version 0
    0x00,
...
QUnit.test("produces an empty array for empty input", function (assert) {
  assert.strictEqual(
    mp4.tools.inspect(new Uint8Array([])).length,
    0,
    ...
    mp4.tools.inspect(
      new Uint8Array(
        box(
          ...
          0x00,
          0x04,
          0x00, // 1024 = 0x400 bytes/second rate
          0x00,
          0x00,
          0x00,
          0x01 // initial delay
...
QUnit.test("can parse an mdat", function (assert) {
  ...
  assert.deepEqual(
    mp4.tools.inspect(mdat),
    [
      {
        size: 16,
        type: "mdat",
        nals: ["slice_layer_without_partitioning_rbsp"],
        byteLength: 8,
      },
    ],
    "parsed an mdat"
...
QUnit.test("can parse a free or skip", function (assert) {
  var free = new Uint8Array(box("free", 0x01, 0x02, 0x03, 0x04)), // data
    skip = new Uint8Array(box("skip", 0x01, 0x02, 0x03, 0x04)); // data

  assert.deepEqual(
    mp4.tools.inspect(free),
    [
      {
        size: 12,
        type: "free",
...
QUnit.test("can parse an mvex", function (assert) {
  var mvex = box(
    ...
    box(
      ...
      0x00,
      0x00,
      0x02, // default_sample_duration
      0x00,
      0x00,
      0x00,
      0x03, // default_sample_size
      0x00,
      0x61,
      0x00,
      0x01
  ...
  assert.deepEqual(
    mp4.tools.inspect(new Uint8Array(mvex)),
    [
      {
        type: "mvex",
        size: 40,
  ...
  var data = box(
    ...
    box(
      ...
      box(
        ...
        0x00,
        0x01, // sequenceParameterSetLength
        0x00, // "SPS"
        0x02, // numOfPictureParameterSets
        0x00,
        0x02, // pictureParameterSetLength
        0x01,
        0x02, // "PPS"
        0x00,
        0x01, // pictureParameterSetLength
        0xff
      ), // "PPS"
      box(
        "btrt",
          ...
          config: [
            {
              ...
              pps: [new Uint8Array([1, 2]), new Uint8Array([0xff])],
            },
            {
              type: "btrt",
              size: 20,
...
QUnit.test("can parse an audio stsd", function (assert) {
  var data = box(
    ...
    box(
      ...
      0x80,
      0x00,
      0x00, // samplerate, fixed-point 16.16
      box(
        "esds",
        0x00, // version 0
        0x00,
  ...
  assert.deepEqual(
    mp4.tools.inspect(new Uint8Array(data)),
    [
      {
        version: 0,
        flags: new Uint8Array([0, 0, 0]),
...
QUnit.test("can parse a trun", function (assert) {
  var data = box(
    ...
    0x00,
    0x00,
    0x00, // sample_composition_time_offset

    0x00,
    0x00,
    0x00,
    0x08, // sample_duration
    0x00,
    0x00,
    0x00,
    0xfe, // sample_size
...
QUnit.test("can parse a sidx", function (assert) {
  var data = box(
    ...
    0x02,
    0x03,
    0x04, // earliest_presentation_time
    0x00,
    0x00,
    0x00,
    0x00, // first_offset
    ...
    0x00,
    0x00,
    0x07, // reference_type(1) + referenced_size(31)
    0x00,
    0x00,
    0x00,
    0x08, // subsegment_duration
    0x80,
    0x00,
    0x00,
    0x09, // starts_with_SAP(1) + SAP_type(3) + SAP_delta_time(28)
    // second reference
    0x00,
    0x00,
    0x00,
    0x03, // reference_type(1) + referenced_size(31)
    0x00,
    0x00,
    0x00,
    0x04, // subsegment_duration
    0x10,
    0x00,
    0x00,
    0x05 // starts_with_SAP(1) + SAP_type(3) + SAP_delta_time(28)
  );
  assert.deepEqual(mp4.tools.inspect(new Uint8Array(data)), [
    {
      type: "sidx",
...
QUnit.test("can parse a version 1 sidx", function (assert) {
  var data = box(
    ...
    0x00,
    0x00,
    0x00, // earliest_presentation_time
    0x01,
    0x02,
    0x03,
    0x04,
...
QUnit.test(
  "can parse a version 1 tfdt and return an unsigned integer value",
  function (assert) {
    var data = box(
      "tfdt",
      0x01, // version
...
QUnit.test("can parse a series of boxes", function (assert) {
  ...
  assert.deepEqual(
    mp4.tools.inspect(new Uint8Array(ftyp.concat(ftyp))),
    [
      {
        type: "ftyp",
        size: 4 * 6,
