...
QUnit.test("correctly determines aac data", function (assert) {
  ...
  assert.ok(
    utils.isLikelyAacData(segmentOnly),
    "test segment is aac without id3"
  );
  assert.notOk(
    utils.isLikelyAacData(testSegment.subarray(id3Offset + 25)),
    "non aac data not recognized"
  );
  assert.notOk(
    utils.isLikelyAacData(testSegment.subarray(0, 5)),
    "not enough aac data is not recognized"
  );
  assert.ok(
    utils.isLikelyAacData(multipleId3),
    "test segment with multilpe id3"
  );
});

QUnit.test("correctly parses aac packet type", function (assert) {
  assert.equal(
    utils.parseType(testSegment, id3TagOffset),
    "timed-metadata",
    "parsed timed-metadata type"
  );
  assert.equal(utils.parseType(testSegment, 1), null, "parsed unknown type");
  assert.equal(
    utils.parseType(testSegment, audioFrameOffset),
    "audio",
    "parsed audio type"
  );
});
...
QUnit.test("correctly parses ID3 tag size", function (assert) {
  assert.equal(
    utils.parseId3TagSize(testSegment, id3TagOffset),
    73,
...
QUnit.test("correctly parses adts frame size", function (assert) {
  assert.equal(
    utils.parseAdtsSize(testSegment, audioFrameOffset),
    13,
...
QUnit.test("correctly parses packet sample rate", function (assert) {
  ...
  var frame = testSegment.subarray(
    audioFrameOffset,
    audioFrameOffset + frameSize
  );
...
QUnit.test("parses correct ID3 tag size", function (assert) {
  ...
  assert.equal(
    utils.parseId3TagSize(packetStream, 0),
    73,
...
QUnit.test("parses correct ADTS Frame size", function (assert) {
  ...
  assert.equal(
    utils.parseAdtsSize(packetStream, 0),
    239,
    "correctly parsed framesize"
  );
});
