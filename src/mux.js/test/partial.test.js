...
[
  ...
  QUnit.test(
    "Audio frames after video not trimmed, " + name,
    function (assert) {
      var segments = [],
        earliestDts = 15000,
      ...
      transmuxer.push(packetize(audioPes([0x19, 0x47], true, earliestDts + 1)));
      transmuxer.push(
        packetize(
          videoPes(
            [
              0x09,
              0x01, // access_unit_delimiter_rbsp
            ],
            true,
            earliestDts
          )
      ...
      transmuxer.push(
        packetize(
          videoPes(
            [
              0x05,
              0x01, // slice_layer_without_partitioning_rbsp_idr
            ],
            true,
            earliestDts
          )
      ...
      assert.equal(
        segments.length,
        2,
        "generated a video and an audio segment"
      );
  ...
  QUnit.test("Audio frames trimmed before video, " + name, function (assert) {
    ...
    transmuxer.push(
      packetize(audioPes([0x19, 0x47], true, earliestDts - baseTime - 1))
    );
    transmuxer.push(
      packetize(
        videoPes(
