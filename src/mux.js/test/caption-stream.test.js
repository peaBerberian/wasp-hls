...
var window = require("global/window"),
  ...
  makeSeiFromMultipleCaptionPackets =
    seiNalUnitGenerator.makeSeiFromMultipleCaptionPackets,
...
QUnit.test(
  "parses SEIs containing multiple messages of type user_data_registered_itu_t_t35",
  function (assert) {
    var packets = [],
      data;
    ...
    assert.equal(
      packets.length,
      1,
      "ignored DTG1 payload, parsed a GA94 caption"
    );
  }
);
...
QUnit.test(
  "ignores SEIs that do not have type user_data_registered_itu_t_t35",
  function (assert) {
    var captions = [];
...
QUnit.test("can be parsed from a segment", function (assert) {
  ...
  assert.equal(
    captions[0].text.indexOf("ASUKA"),
    0,
    "parsed the start of the first caption"
  );
  assert.ok(
    captions[0].text.indexOf("Japanese") > 0,
    "parsed the end of the first caption"
  );
...
QUnit.test("dispatches caption track information", function (assert) {
  ...
  assert.deepEqual(
    captionStreams,
    { CC1: true, CC3: true },
    "found captions in CC1 and CC3"
  );
  assert.equal(captions.length, 4, "parsed eight captions");
  assert.equal(
    captions[0].text,
    "être une période de questions",
    "parsed the text of the first caption in CC3"
  );
  assert.equal(
    captions[1].text,
    "PERIOD, FOLKS.",
    "parsed the text of the first caption in CC1"
  );
});
...
QUnit.test(
  "drops duplicate segments with multi-segment DTS values",
  function (assert) {
    var packets, captions, seiNals;
    packets = [
      {
    ...
    assert.equal(
      captions[0].text,
      "test string data stuff",
      "parsed caption properly"
    );
    assert.equal(
      captions[1].text,
...
QUnit.test(
  "drops data until first command that sets activeChannel for a field",
  function (assert) {
    var captions = [];
    captionStream.ccStreams_.forEach(function (cc) {
      cc.on("data", function (caption) {
    ...
    assert.equal(
      captions[0].text,
      "field0",
      "received only confirmed field0 data"
    );
    assert.equal(captions[0].stream, "CC1", "caption went to right channel");
    assert.equal(
      captions[1].text,
      "field1",
      "received only confirmed field1 data"
    );
...
QUnit.test(
  "clears buffer and drops data until first command that sets activeChannel after reset",
  function (assert) {
    var firstPackets, secondPackets, captions, seiNals1, seiNals2;
    captions = [];
...
QUnit.test(
  "don't mess up 608 captions when 708 are present",
  function (assert) {
    var captions = [];
    captionStream.ccStreams_.forEach(function (cc) {
      cc.on("data", function (caption) {
    ...
    assert.equal(
      captions[2].text,
      "WE TRY NOT TO PUT AN ANIMAL DOWN\nIF WE DON'T HAVE TO.",
...
QUnit.test(
  "both 608 and 708 captions are available by default",
  function (assert) {
    var cc608 = [];
    var cc708 = [];
    captionStream.on("data", function (caption) {
...
QUnit.test("ignores XDS and Text packets", function (assert) {
  ...
  [
    ...
    .map(makeSeiFromCaptionPacket)
    .forEach(captionStream.push, captionStream);
  captionStream.flush();
  assert.equal(
    captionStream.activeCea608Channel_[0],
    0,
    "field 1: CC1 is active"
  );
  assert.equal(
    captionStream.activeCea608Channel_[1],
    0,
    "field 2: CC3 is active"
  );
...
QUnit.test(
  "special and extended character codes work regardless of field and data channel",
  function (assert) {
    var packets,
      seiNals,
      captions = [];
    ...
    assert.deepEqual(
      ...
      "CC2 special character correct"
    );
    assert.deepEqual(
      captions[1].text,
      String.fromCharCode(0xab),
      "CC3 extended character correct"
    );
    assert.deepEqual(
      captions[2].text,
      String.fromCharCode(0xbb),
      "CC4 extended character correct"
    );
  }
);

QUnit.test(
  "number of roll up rows takes precedence over base row command",
  function (assert) {
    var captions = [];
    var packets = [
...
QUnit.test(
  "backspaces are applied to non-displayed memory for pop-on mode",
  function (assert) {
    var captions = [],
      packets;
    cea608Stream.on("data", function (caption) {
...
QUnit.test(
  "recognizes the Erase Non-Displayed Memory command",
  function (assert) {
    var packets, captions;
    packets = [
      // RCL, resume caption loading
...
QUnit.test("closes formatting at PAC row change", function (assert) {
  ...
  assert.equal(
    captions[0].text,
    "<u><i>yes.</i></u>\nno",
    "properly closed by PAC row change"
  );
  assert.deepEqual(cea608Stream.formatting_, [], "formatting is empty");
});
...
QUnit.test("closes formatting at negating mid-row code", function (assert) {
  ...
  assert.equal(
    captions[0].text,
    "no <i><u>yes.</u></i> no",
    "properly closed by negating mid-row code"
  );
  assert.deepEqual(cea608Stream.formatting_, [], "formatting is empty");
});
...
QUnit.test(
  "switching to roll-up from pop-on wipes memories and flushes captions",
  function (assert) {
    var captions = [];
    cea608Stream.on("data", function (caption) {
      captions.push(caption);
...
QUnit.test(
  "switching to roll-up from paint-on wipes memories and flushes captions",
  function (assert) {
    var captions = [];
    cea608Stream.on("data", function (caption) {
      captions.push(caption);
...
QUnit.test(
  "switching to paint-on from pop-on flushes display",
  function (assert) {
    var captions = [];
    cea608Stream.on("data", function (caption) {
      captions.push(caption);
...
QUnit.test(
  "backspaces are reflected in the generated captions",
  function (assert) {
    var captions = [];
    cea608Stream.on("data", function (caption) {
      captions.push(caption);
...
QUnit.test(
  "a second identical control code immediately following the first is ignored",
  function (assert) {
    var captions = [];
    cea608Stream.on("data", function (caption) {
      captions.push(caption);
...
QUnit.test(
  "a second identical control code separated by only padding from the first is ignored",
  function (assert) {
    var captions = [];
    cea608Stream.on("data", function (caption) {
      captions.push(caption);
...
QUnit.test(
  "preamble address codes on same row are NOT converted into spaces",
  function (assert) {
    var captions = [];
    cea608Stream.on("data", function (caption) {
      captions.push(caption);
...
QUnit.test("preserves newlines from PACs in pop-on mode", function (assert) {
  ...
  assert.equal(
    captions[0].text,
    "TEST\n\nSTRING\nDATA",
    "Position PACs were converted to newlines"
  );
});
...
QUnit.test("reset works", function (assert) {
  ...
  var buffer = cea608Stream.displayed_
    .map(function (row) {
      return row.trim();
    })
    .join("\n")
  ...
  assert.equal(
    cea608Stream.lastControlCode_,
    null,
    "last control code reset successfully"
  );
...
QUnit.test(
  "backspaces are reflected in the generated captions (paint-on)",
  function (assert) {
    var captions = [];
    cea608Stream.on("data", function (caption) {
      captions.push(caption);
...
QUnit.test(
  "Cea608Stream will trigger log on malformed captions",
  function (assert) {
    var result;
    var logs = [];
    ...
    assert.ok(result, "the function does not throw an exception");
    assert.deepEqual(
      logs,
      [
        {
          level: "warn",
          message: "Skipping a malformed 608 caption at index 0.",
        },
      ],
      "logs were triggered"
    );
...
QUnit.test(
  "Filters encoding values out of captionServices option block",
  function (assert) {
    ...
    assert.deepEqual(
      cea708Stream.serviceEncodings,
      expectedServiceEncodings,
      "filtered encodings correctly"
    );
  }
);
...
QUnit.test(
  "Decodes multibyte characters if valid encoding option is provided and TextDecoder is supported",
  function (assert) {
    var captions = [];
    ...
    if (window.TextDecoder) {
      assert.ok(
        cea708Stream.services[1].textDecoder_,
        "TextDecoder created when supported"
      );
      assert.equal(
        captions[0].text,
        "니가 ",
      ...
      assert.notOk(
        cea708Stream.services[1].textDecoder_,
        "TextDecoder not created when unsupported"
      );
    }
  }
);
...
QUnit.test(
  "Creates TextDecoder only if valid encoding value is provided",
  function (assert) {
    ...
    if (window.TextDecoder) {
      assert.ok(
        cea708Stream.services[1].textDecoder_,
        "TextDecoder created successfully when encoding is valid"
      );
    }
    ...
    secondCea708Stream.flushDisplayed(
      4721138662,
      secondCea708Stream.services[1]
    );

    assert.notOk(
      secondCea708Stream.services[1].textDecoder_,
      "TextDecoder not created when encoding is invalid"
    );
  }
);
...
QUnit.test("backspace", function (assert) {
  ...
  assert.equal(
    captions[0].text,
    "typo",
    "parsed caption with backspaces correctly"
  );
});
...
QUnit.test("extended character set", function (assert) {
  ...
  assert.equal(
    captions[0].text,
    "Ÿ•Ÿ œŠ…Šœ “℠”™",
    "parsed extended characters correctly"
  );
});
