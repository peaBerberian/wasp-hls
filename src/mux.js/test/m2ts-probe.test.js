...
QUnit.test(
  "correctly parses program map table from pmt packet",
  function (assert) {
    assert.deepEqual(
      probe.parsePmt(pmtPacket),
      programMapTable,
      "generates correct pmt"
    );
  }
);
...
QUnit.test("correctly parses payload unit start indicator", function (assert) {
  assert.ok(
    probe.parsePayloadUnitStartIndicator(pesPacket),
    "detects payload unit start indicator"
  );
  assert.ok(
    !probe.parsePayloadUnitStartIndicator(notPusiPacket),
...
QUnit.test("correctly parses type of pes packet", function (assert) {
  assert.equal(
    probe.parsePesType(videoPacket, programMapTable),
    "video",
    "parses video pes type"
  );
  assert.equal(
    probe.parsePesType(audioPacket, programMapTable),
    "audio",
...
QUnit.test(
  "correctly parses dts and pts values of pes packet",
  function (assert) {
...
QUnit.test(
  "correctly determines if video pes packet contains a key frame",
  function (assert) {
    assert.ok(
      probe.videoPacketContainsKeyFrame(videoPacket),
      "detects key frame in packet"
    );
    assert.ok(
      !probe.videoPacketContainsKeyFrame(videoNoKeyFramePacket),
