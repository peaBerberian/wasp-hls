...
export interface AdtsStreamEvents {
  data: AacFrame;
  done: null;
  partialdone: null;
  reset: null;
  ...
  public parsePacket(packet: AdtsPacket): AacFrame[] {
    ...
    if (buffer !== null && buffer.length > 0) {
      ...
      buffer = new Uint8Array(
        oldBuffer.byteLength + packet.data.byteLength
      );
...
export function adtsPacketsToAacFrames(
  packets: AdtsPacket[],
  previousUnfinishedState: UnprocessedAdtsPacketsData | null,
): [AacFrame[], UnprocessedAdtsPacketsData] {
  let buffer = previousUnfinishedState?.buffer ?? null;
  const frames: AacFrame[] = [];
 ...
 */
class AdtsStream extends EventEmitter<AdtsStreamEvents> {
  private _handlePartialSegment: boolean;
  private _buffer: Uint8Array | null;
  private _frameNum: number;
  constructor(handlePartialSegments: boolean = false) {
    super();
    this._handlePartialSegment = handlePartialSegments;
    this._frameNum = 0;
    this._buffer = null;
  }

  public push(packet: AdtsPacket): void {
    if (!this._handlePartialSegment) {
      this._frameNum = 0;
    }
    ...
    if (this._buffer !== null && this._buffer.length > 0) {
      ...
      this._buffer.set(packet.data, oldBuffer.byteLength);
    } else {
      this._buffer = packet.data;
    }
    ...
    while (i + 7 < buffer.length) {
      ...
      // Otherwise, deliver the complete AAC frame
      this.trigger("data", {
        pts: packet.pts + this._frameNum * adtsFrameDuration,
        dts: packet.dts + this._frameNum * adtsFrameDuration,
        sampleCount,
        audioobjecttype: ((buffer[i + 2] >>> 6) & 0x03) + 1,
        channelcount:
          ((buffer[i + 2] & 1) << 2) | ((buffer[i + 3] & 0xc0) >>> 6),
  ...
  private _skipWarn(start: number, end: number): void {
    logger.warn(
      `Transmux: adts skiping bytes ${start} to ${end} ` +
        `in frame ${this._frameNum} outside syncword`
    );
  }
}
